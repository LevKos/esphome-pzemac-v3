...
namespace esphome
{
  namespace mymodbus
  {
    ...
    static const char *const TAG = "mymodbus";
    ...
    bool MyModbus::parse_modbus_byte_(uint8_t byte)
    {
      size_t at = this->rx_buffer_.size();
      this->rx_buffer_.push_back(byte);
      const uint8_t *raw = &this->rx_buffer_[0];
      ESP_LOGVV(TAG, "MyModbus received Byte  %d (0X%x)", byte, byte);
           ...
           (function_code <= FUNCTION_CODE_USER_DEFINED_SPACE_2_END)))
      {
        ...
        ESP_LOGD(TAG, "MyModbus user-defined function %02X found", function_code);
      }
      else
      {
        // data starts at 2 and length is 4 for read registers commands
        if (this->role == MyModbusRole::SERVER)
        {
        ...
        if (computed_crc != remote_crc)
        {
          ...
          {
            ESP_LOGD(TAG, "MyModbus CRC Check failed, but ignored! %02X!=%02X", computed_crc, remote_crc);
          }
          else
          {
            ESP_LOGW(TAG, "MyModbus CRC Check failed! %02X!=%02X", computed_crc, remote_crc);
            return false;
          }
        }
      }
      ...
      for (auto *device : this->devices_)
      {
        if (device->address_ == address)
        {
          ...
          if ((function_code & FUNCTION_CODE_EXCEPTION_MASK) == FUNCTION_CODE_EXCEPTION_MASK)
          {
            ESP_LOGD(TAG, "NyModbus error function code: 0x%X exception: %d", function_code, raw[2]);
            if (waiting_for_response != 0)
            {
    ...
    void MyModbus::dump_config()
    {
      ESP_LOGCONFIG(TAG, "MyModbus:");
    ...
    float MyModbus::get_setup_priority() const
    {
      // After UART bus
      return setup_priority::BUS - 1.0f;
    }

    void MyModbus::send(uint8_t address, uint8_t function_code, uint16_t start_address, uint16_t number_of_entities,
                        uint8_t payload_len, const uint8_t *payload)
    {
      ...
      if (this->role == MyModbusRole::CLIENT)
      {
      ...
      if (payload != nullptr)
      {
        if (this->role == MyModbusRole::SERVER || function_code == ModbusFunctionCode::WRITE_MULTIPLE_COILS ||
            function_code == ModbusFunctionCode::WRITE_MULTIPLE_REGISTERS)
        {                              // Write multiple
      ...
      last_send_ = millis();
      ESP_LOGV(TAG, "MyModbus write: %s", format_hex_pretty(data).c_str());
    }

    // Helper function for lambdas
    // Send raw command. Except CRC everything must be contained in payload
    void MyModbus::send_raw(const std::vector<uint8_t> &payload)
    {
      if (payload.empty())
      {
        return;
      ...
      ESP_LOGV(TAG, "MyModbus write raw: %s", format_hex_pretty(payload).c_str());
      last_send_ = millis();
    }
